...
Kipe kila kipengele cha `section` jukumu la `region`.

# --hints--

Unapaswa kukipa kipengele cha kwanza cha `section` jukumu la `region`.

```js
assert.equal(document.querySelectorAll('section')?.[0]?.getAttribute('role'), 'region');
```

Unapaswa kukipa kipengele cha pili cha `section` jukumu la `region`.

```js
assert.equal(document.querySelectorAll('section')?.[1]?.getAttribute('role'), 'region');
```

Unapaswa kukipa kipengele cha tatu cha `section` jukumu la `region`.
