...
Pia, weka maandishi ya kipengele `label` upande kulia.

# --hints--

Unapaswa kutumia aidha kichaguzi cha `.info > label, .info > input` au `.info label, .info input`.
...
Unapaswa kuvipa vipengele vya `input` na `label` `display` ya `inline-block`.
...
Unapaswa kukipa kipengele cha `label` `text-align` ya `right`.
