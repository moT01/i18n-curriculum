...
Unapaswa kuweka kipengele kimoja cha `label` ndani ya kipengele cha kwanza cha `div.question-block`.

```js
assert.exists(document.querySelectorAll('.formrow > .question-block')?.[0]?.querySelector('label'));
```

Unapaswa kuweka kipengele kimoja cha `label` ndani ya kipengele cha pili cha `div.question-block`.

```js
assert.exists(document.querySelectorAll('.formrow > .question-block')?.[1]?.querySelector('label'));
```

Unapaswa kukipa kipengele cha kwanza cha `label` maudhui ya maandishi.

```js
assert.isNotEmpty(document.querySelectorAll('.formrow > .question-block')[0]?.querySelector('label')?.textContent.trim());
```

Unapaswa kukipa kipengele cha pili cha `label` maudhui ya maandishi.
