...
Unapaswa kukipa kipengele cha kwanza cha `a` `href` ya `#student-info`.

```js
assert.equal(document.querySelectorAll('a')?.[0]?.getAttribute('href'), '#student-info');
```

Unapaswa kukipa kipengele cha pili cha `a` `href` ya `#html-questions`.

```js
assert.equal(document.querySelectorAll('a')?.[1]?.getAttribute('href'), '#html-questions');
```

Unapaswa kukipa kipengele cha tatu cha `a` `href` ya `#css-questions`.
