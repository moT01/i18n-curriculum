...
Unapaswa kuweka vipengele viwili vya `div` ndani ya kipengele cha pili cha `section`.

```js
assert.equal(document.querySelectorAll('section:nth-of-type(2) > div')?.length, 2);
```

Unapaswa kukipa kipengee kipya cha kwanza cha `div` darasa la `question-block`.

```js
assert.equal(document.querySelectorAll('section:nth-of-type(2) > div')?.[0]?.className, 'question-block');
```

Unapaswa kukipa kipengele kipya cha pili cha `div` darasa la `question-block`.
...
Unapaswa kuweka kipengele kimoja cha `fieldset` ndani ya kila kipengele cha `div.question-block`.
...
Unapaswa kukipa kipengele cha kwanza cha `fieldset` darasa la `question`.

```js
assert.equal(document.querySelectorAll('section:nth-of-type(2) > div.question-block > fieldset')?.[0]?.className, 'question');
```

Unapaswa kukipa kipengele cha pili cha `fieldset`darasa la `question`.
