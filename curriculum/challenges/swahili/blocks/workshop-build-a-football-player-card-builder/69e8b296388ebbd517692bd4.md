...
Add two more `div` elements each with the `className` of `form-group` inside `stats-grid`, one for PAS and the other for DRI.

For that of PAS, add a `label` with an `htmlFor` of `pas` and the text `PAS`, then an input with an `id` of `pas` and a `type` of `number`.

And for that of DRI, add a `label` with an `htmlFor` of `dri` and the text `DRI`, then an input with an `id` of `dri` and a `type` of `number`.
...
```js
const container = await __helpers.prepTestComponent(window.index.FootballPlayerCard);
const statsGridDiv = container.querySelectorAll('.form-panel > div > div.stats-grid > div.form-group')[2];
const label = statsGridDiv.querySelector('label.label[for="pas"]');
assert.exists(label);
...
```js
const container = await __helpers.prepTestComponent(window.index.FootballPlayerCard);
const statsGridDiv = container.querySelectorAll('.form-panel > div > div.stats-grid > div.form-group')[3];
const label = statsGridDiv.querySelector('label.label[for="dri"]');
assert.exists(label);
...
assert.exists(statsGridDiv.querySelector('input#dri.input[type="number"]'));
```
