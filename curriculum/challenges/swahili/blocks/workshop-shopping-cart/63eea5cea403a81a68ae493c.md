...
```js
const cart = new ShoppingCart();
assert.match(cart.addItem.toString(), /products\.find\(\s*(?:function\s*\(|\(?\s*item\s*\)?\s*=>)/);
```
...
```js
const cart = new ShoppingCart();
assert.match(cart.addItem.toString(), /products\.find\(\s*(?:function\s*\(\s*item\s*\)|\(?\s*item\s*\)?\s*=>)/);
```
...
assert.match(
  ...
  /products\.find\(\s*(?:function\s*\(\s*item\s*\)\s*\{\s*return\s+|\(?\s*item\s*\)?\s*=>\s*)(?:item(?:\.id|\[\s*['"]id['"]\s*\])\s*===\s*id|id\s*===\s*item(?:\.id|\[\s*['"]id['"]\s*\]))/
);
```
