...
Unapaswa kuunda kichaguzi kipya cha `50%` katika sheria yako ya `@keyframes cabins`.
...
Kichaguzi chako cha `50%` kinapaswa kuwa kati ya viteuzi vyako vya `0%` na `100%`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getCSSRules('keyframes')?.[1]?.cssRules?.[1]?.keyText, '50%');
```

Kichaguzi chako cha `50%` kinafaa kuwa na `background-color` iliyowekwa kuwa `purple`.
