...
Kichaguzi chako cha `.line` kinafaa kuwa na sifa ya `position` iliyowekwa kuwa `absolute`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.line')?.position, 'absolute');
```

Kichaguzi chako cha `.line` kinafaa kuwa na sifa ya `left` iliyowekwa kuwa `50%`.

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('.line')?.left, '50%');
```

Kichaguzi chako cha `.line` kinafaa kuwa na sifa ya `top` iliyowekwa kuwa `50%`.
