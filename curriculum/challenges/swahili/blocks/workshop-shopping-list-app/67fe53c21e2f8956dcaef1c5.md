...
assert.match(shoppingListString, /setSelectedItems\s*\(\s*(?:function\s*\(\s*prev\s*\)|\(?\s*prev\s*\)?\s*=>)/);
```
...
assert.match(shoppingListString, /(?:function\s*\(\s*prev\s*\)\s*{\s*return|\(\s*prev\s*\)\s*=>|prev\s*=>)[\s\S]*prev\.includes\s*\(\s*item\s*\)/);
```
