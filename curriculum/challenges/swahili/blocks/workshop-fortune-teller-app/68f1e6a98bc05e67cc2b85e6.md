...
Declare a variable named `cardElement` and use the `card_container` class to get the element. Use the `closest` selector to get it from the `target`.

# --hints--

You should declare a variable named `cardElement` inside `showFortune`.

```js
const explorer = await __helpers.Explorer(code);
const method = explorer.classes.Game.methods.showFortune.toString();
assert.match(method, /const\s+cardElement\s*=/);
```

Your `cardElement` should use `target?.closest` to find the `.card_container`.

```js
const explorer = await __helpers.Explorer(code);
const method = explorer.classes.Game.methods.showFortune.toString();
assert.match(method, /cardElement\s*=\s*target\?\.closest\s*\(\s*['"]\.card_container['"]\s*\)/);
```

You should pass `.card_container` as the argument to `closest`.

```js
assert.match(__helpers.removeJSComments(code), /\.closest\s*\(\s*['"]\.card_container['"]\s*\)/);
```
