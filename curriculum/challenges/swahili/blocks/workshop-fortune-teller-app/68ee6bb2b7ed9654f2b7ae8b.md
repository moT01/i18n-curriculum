...
Add another event listener to the global `document` object that calls the `showFortune` function. Inside the callback, include a parameter `e` of type `Event`, and pass `e` as an argument to the `showFortune` method.

`showFortune` will handle card click events, like reading the clicked card ID from the DOM and displaying the matching card fortune details. You will create it in the coming steps.

# --hints--

You should assign a `click` event listener to the global `document`.

```js
assert.match(__helpers.removeJSComments(code), /document\s*\.\s*addEventListener\s*\(\s*['"]click['"]/);
```

Your event listener should have a callback parameter named `e`.

```js
assert.match(__helpers.removeJSComments(code), /addEventListener\s*\([\s\S]*?\(\s*e\s*[:)]/);
```

Your callback parameter should have the type `Event`.

```js
assert.match(__helpers.removeJSComments(code), /\(\s*e\s*:\s*Event\s*\)/);
```

Inside the callback, you should call the `showFortune` function

```js
assert.match(__helpers.removeJSComments(code), /addEventListener[\s\S]*?\(\s*[^)]*\)\s*=>\s*(\{[\s\S]*?this\.showFortune\s*\(|this\.showFortune\s*\()/);
```

Inside the `showFortune` function, you should pass the argument `e`.

```js
assert.match(__helpers.removeJSComments(code), /showFortune\s*\(\s*e\s*\)/);
```
