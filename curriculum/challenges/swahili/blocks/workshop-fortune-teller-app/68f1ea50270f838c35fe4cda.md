...
Now do the same for `subTitle` by assigning it to `meaning_up`, and for `title` assign it to `foundCard.name`. After that, call the `showElements` method and pass `fortuneDescription` from the elements.

# --hints--

You should set `this.elements.subTitle.textContent` to `foundCard.meaning_up`.

```js
const explorer = await __helpers.Explorer(code);
const method = explorer.classes.Game.methods.showFortune.toString();
assert.match(method, /this\.elements\.subTitle\.textContent\s*=\s*foundCard\.meaning_up/);
```

You should set `this.elements.title.textContent` to `foundCard.name`.

```js
const explorer = await __helpers.Explorer(code);
const method = explorer.classes.Game.methods.showFortune.toString();
assert.match(method, /this\.elements\.title\.textContent\s*=\s*foundCard\.name/);
```

You should call the `showElements` function inside `showFortune`.

```js
const explorer = await __helpers.Explorer(code);
const method = explorer.classes.Game.methods.showFortune.toString();
assert.match(method, /showElements\s*\(/);
```

You should pass `this.elements.fortuneDescription` as an argument to `showElements`.

```js
const explorer = await __helpers.Explorer(code);
const method = explorer.classes.Game.methods.showFortune.toString();
assert.match(method, /showElements\s*\([\s\S]*?this\.elements\.fortuneDescription/);
```
