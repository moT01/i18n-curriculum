...
Declare another variable named `foundCard`, and assign it to the result of calling the `find()` method on `cards`. Pass in `card` as the callback parameter, and compare `card.name_short` with `cardId` to find the matching card using strict equality.

# --hints--

You should declare a variable named `foundCard` inside `showFortune`.

```js
const explorer = await __helpers.Explorer(code);
const method = explorer.classes.Game.methods.showFortune.toString();
assert.match(method, /const\s+foundCard\s*=/);
```

Your `foundCard` should be assigned by calling `find` on `this.cards` with the correct callback.

```js
const explorer = await __helpers.Explorer(code);
const method = explorer.classes.Game.methods.showFortune.toString();
assert.match(method, /foundCard\s*=\s*this\.cards\.find\s*\(/);
```

The `find` callback should have a parameter named `card`.

```js
assert.match(__helpers.removeJSComments(code), /cards\.find\s*\(\s*\(\s*card\s*\)\s*=>/);
```

Inside the callback, `card.name_short` should be compared with `cardId`.

```js
assert.match(__helpers.removeJSComments(code), /card\.name_short\s*===?\s*cardId/);
```
