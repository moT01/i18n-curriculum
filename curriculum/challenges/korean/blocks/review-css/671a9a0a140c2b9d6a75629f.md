...
- **Shopping Cart**: Carts are a place for users to see what items they have already selected on an e-commerce platform. Your carts should always be visible to the user, use a common icon like a cart, bag or basket, and have a clear call-to-action button for users to proceed to checkout.
...
- **`pt` (Points)**: This absolute unit is equal to 1/72nd of an inch.
...
- **`flex-wrap: nowrap;`**: This is the default value. Flex items won't be wrapped onto a new line, even if their width exceeds the container's width.
