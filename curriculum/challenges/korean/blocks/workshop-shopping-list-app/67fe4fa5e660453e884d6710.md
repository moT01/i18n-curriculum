...
assert.match(shoppingListString, /toggleItem\s*=\s*(?:function\s*\(\s*item\s*\)|\(?\s*item\s*\)?\s*=>)/);
```
...
assert.match(shoppingListString, /toggleItem\s*=\s*(?:function\s*\(\s*item\s*\)|\(?\s*item\s*\)?\s*=>)\s*\{[\s\S]*setSelectedItems\s*\(/);
```
