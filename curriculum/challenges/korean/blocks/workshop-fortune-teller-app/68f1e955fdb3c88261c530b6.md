...
Add an `if` statement to check whether `foundCard` exists before updating the elements with its data or content block.

# --hints--

You should declare an `if` statement with the condition `foundCard`.

```js
assert.match(__helpers.removeJSComments(code), /if\s*\(\s*foundCard\s*\)\s*\{/);
```
