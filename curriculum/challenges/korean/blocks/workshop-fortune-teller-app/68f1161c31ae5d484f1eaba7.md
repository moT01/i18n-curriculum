...
Now add another method called `showFortune`. As a parameter, pass `e` and give it the type `Event`. Also create a `newReading` method and leave it empty for now.

# --hints--

You should declare a method named `showFortune`.

```js
const explorer = await __helpers.Explorer(code);
assert.exists(explorer.classes.Game.methods.showFortune);
```

Your `showFortune` method should have an `e` parameter with the type `Event`.

```js
const explorer = await __helpers.Explorer(code);
const { showFortune } = explorer.classes.Game.methods;
assert.isTrue(showFortune.parameters[0].matches("e: Event"));
```

You should declare a method named `newReading`.

```js
const explorer = await __helpers.Explorer(code);
assert.exists(explorer.classes.Game.methods.newReading);
```
