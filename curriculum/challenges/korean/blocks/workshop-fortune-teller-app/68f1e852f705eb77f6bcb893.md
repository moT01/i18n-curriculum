...
Declare a variable named `cardId`, and assign it the value of the `data-id` attribute from `cardElement` using the `getAttribute()` method.

# --hints--

You should declare a variable named `cardId` inside `showFortune`.

```js
const explorer = await __helpers.Explorer(code);
const method = explorer.classes.Game.methods.showFortune.toString();
assert.match(method, /const\s+cardId\s*=/);
```

Your `cardId` variable should be assigned by calling `cardElement.getAttribute("data-id")`.

```js
const explorer = await __helpers.Explorer(code);
const method = explorer.classes.Game.methods.showFortune.toString();
assert.match(method, /cardId\s*=\s*cardElement\.getAttribute\s*\(\s*['"]data-id['"]\s*\)/);
```

You should use the `getAttribute` method on `cardElement`.

```js
assert.match(__helpers.removeJSComments(code), /cardElement\.getAttribute\s*\(/);
```

You should pass `'data-id'` to `getAttribute`.

```js
assert.match(__helpers.removeJSComments(code), /\.getAttribute\s*\(\s*['"]data-id['"]\s*\)/);
```
