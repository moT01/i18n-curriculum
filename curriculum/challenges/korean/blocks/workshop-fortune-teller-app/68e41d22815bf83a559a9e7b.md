...
Now, use the `onload` event to remove the `hidden` class when the image loads and hide the loading placeholder using its `previousElementSibling`.

# --hints--

Your `img` element should have an `onload` handler.
...
Your `onload` handler should use `classList.remove` to remove the `hidden` class. Don't forget you need the `this` keyword here.

```js
assert.match(
  __helpers.removeJSComments(code),
  /onload\s*=\s*"[^"]*this\.classList\.remove\(\s*'hidden'\s*\)/
);
```

Your `onload` handler should set `previousElementSibling.style.display` to `'none'`. Don't forget you need the `this` keyword to do this.

```js
assert.match(
  __helpers.removeJSComments(code),
  /onload\s*=\s*"[^"]*this\.previousElementSibling\.style\.display\s*=\s*'none'/
);
```
