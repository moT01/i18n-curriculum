...
After the DRI `form-group`, add two more `div` elements each with the `className` of `form-group` inside `stats-grid`, one for DEF and the other for PHY.

For that of DEF, add a `label` with an `htmlFor` of `def` and the text `DEF`, then an input with an `id` of `def` and a `type` of `number`.

And for that of PHY, add a label with an `htmlFor` of `phy` and the text `PHY`, then an input with an `id` of `phy` and a `type` of `number`.

# --hints--

Your should add another set of two `div` elements each with the `className` of `form-group` inside the `div` of `className` of `stats-grid`.

```js
const container = await __helpers.prepTestComponent(window.index.FootballPlayerCard);
const statsGridDiv = container.querySelectorAll('.form-panel > div > div.stats-grid > div.form-group');
...
```js
const container = await __helpers.prepTestComponent(window.index.FootballPlayerCard);
const statsGridDiv = container.querySelectorAll('.form-panel > div > div.stats-grid > div.form-group')[4];
const label = statsGridDiv.querySelector('label.label[for="def"]');
assert.exists(label);
...
```js
const container = await __helpers.prepTestComponent(window.index.FootballPlayerCard);
const statsGridDiv = container.querySelectorAll('.form-panel > div > div.stats-grid > div.form-group')[5];
const label = statsGridDiv.querySelector('label.label[for="phy"]');
assert.exists(label);
assert.include(label.textContent.trim(), 'PHY');
```

Your sixth `div` element of `className` of `form-group` should have an `input` with a `className` of `input`, an `id` of `phy` and a `type` of `number`.

```js
const container = await __helpers.prepTestComponent(window.index.FootballPlayerCard);
const statsGridDiv = container.querySelectorAll('.form-panel > div > div.stats-grid > div.form-group')[5];

assert.exists(statsGridDiv.querySelector('input#phy.input[type="number"]'));
```
