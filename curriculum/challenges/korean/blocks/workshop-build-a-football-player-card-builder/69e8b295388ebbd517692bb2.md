...
Inside the `FootballPlayerCard` component, remove the `h1` and return the `PlayerCard` component.

# --hints--

The `FootballPlayerCard` should render the `PlayerCard` component.

```js
const element = window.index.FootballPlayerCard();
assert.equal(element?.type, window.index.PlayerCard);
```
