...
Now add a `div` with the `className` of `form-group`. Inside it, add a `label` with the `className` of `label` and an `htmlFor` attribute of `name`. Under that add an `input` with an `id` set to `name`, a `className` of `input`, and a `type` attribute of `text`.

# --hints--

Your should have a `div` with the `className` of `form-group`.

```js
const container = await __helpers.prepTestComponent(window.index.FootballPlayerCard);
assert.exists(container.querySelector('.form-panel div.form-group'));
```

Your `form-group` should contain a `label` with the `className` of `label` and a `htmlFor` attribute of `name`.

```js
const container = await __helpers.prepTestComponent(window.index.FootballPlayerCard);
const label = container.querySelector('label.label[for="name"]');
assert.exists(label);
```

Your `form-group` should contain an `input` with an `id` of `name`, `className` of `input`, and a `type` of `text`.

```js
const container = await __helpers.prepTestComponent(window.index.FootballPlayerCard);
const input = container.querySelector('input#name.input[type="text"]');
assert.exists(input);
```
