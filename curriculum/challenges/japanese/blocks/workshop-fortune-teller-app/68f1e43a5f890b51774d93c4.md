...
Inside `showFortune` method declare a variable called `target` and assign it to `e.target` and create another empty if statement.

# --hints--

You should declare a variable named `target` inside `showFortune`.

```js
const explorer = await __helpers.Explorer(code);
const method = explorer.classes.Game.methods.showFortune.toString();
assert.match(method, /const\s+target\s*=/);
```

Your `target` variable should be assigned to `e.target`.

```js
const explorer = await __helpers.Explorer(code);
const method = explorer.classes.Game.methods.showFortune.toString();
assert.match(method, /target\s*=\s*e\.target/);
```

You should have an empty `if` statement right after your `target` variable.

```js
assert.match(__helpers.removeJSComments(code), /showFortune\s*\([^)]*\)\s*\{[\s\S]*?if\s*\(\s*\)\s*\{\s*\}/);
```
