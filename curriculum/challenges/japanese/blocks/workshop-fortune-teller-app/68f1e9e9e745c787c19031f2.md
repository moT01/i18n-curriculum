...
Inside the `if` statement, get the `cardTitle` element and set its `textContent` to `foundCard.name`. Do the same for the `description` element by setting its `textContent` to `foundCard.desc`.

# --hints--

You should set the `cardTitle` `textContent` to `foundCard.name`. Don't forget to use the `this` keyword.

```js
assert.match(__helpers.removeJSComments(code), /cardTitle\.textContent\s*=\s*foundCard\.name/);
```

You should set the `description` `textContent` to `foundCard.desc`. Don't forget to use the `this` keyword.

```js
assert.match(__helpers.removeJSComments(code), /description\.textContent\s*=\s*foundCard\.desc/);
```
