...
`img` セレクターを作成し、`width` プロパティを `100%` に、`object-fit` プロパティを `cover` に設定してください。

`object-fit` プロパティは、コンテナー内で要素をどのように配置するかをブラウザーに指示します。 この例の `cover` は、画像がコンテナー全体を埋めつくすように設定し、アスペクト比が変わらないように必要に応じてクロッピングします。
...
`img` セレクターの `object-fit` プロパティを `cover` に設定する必要があります。
