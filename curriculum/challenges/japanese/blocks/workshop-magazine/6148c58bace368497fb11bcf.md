...
フレックスボックスと同様に、CSS グリッドでも `align-items` および `justify-items` を使用してグリッドアイテムのコンテンツを整列させることができます。 `align-items` は子要素を列方向の軸に沿って配置し、`justify-items` は子要素を行方向の軸に沿って配置します。

`.social-icons` セレクターの `align-items` プロパティを `center` に設定してください。

# --hints--

`.social-icons` セレクターの `align-items` プロパティを `center` に設定する必要があります。
