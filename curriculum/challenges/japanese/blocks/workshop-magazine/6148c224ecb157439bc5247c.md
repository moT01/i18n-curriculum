...
フレックスボックスの場合と異なり、CSS グリッドではデフォルトで、必要に応じて追加の行が作成されます。 `.social-icons` セレクターの `grid-template-columns` プロパティを `repeat(5, 1fr)` に設定して、アイコンを 1 行に並べてください。

# --hints--

`.social-icons` セレクターの `grid-template-columns` プロパティを `repeat(5, 1fr)` に設定する必要があります。
