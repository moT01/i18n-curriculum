...
`.text` セレクターの `font-size` プロパティを `1.8rem` に設定し、`letter-spacing` プロパティを `0.6px` に設定してください。

# --hints--

`.text` セレクターの `font-size` プロパティを `1.8rem` に設定する必要があります。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.text')?.fontSize, '1.8rem');
```

`.text` セレクターの `letter-spacing` プロパティを `0.6px` に設定する必要があります。
