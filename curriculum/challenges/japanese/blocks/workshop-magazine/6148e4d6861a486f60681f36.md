...
画像は 2 列 3 行のレイアウトにします。

`.image-wrapper` セレクターの `grid-template-columns` プロパティを `2fr 1fr` に設定し、 `grid-template-rows` プロパティを `repeat(3, min-content)` に設定してください。 こうすることで、グリッドの行はコンテンツに基づいて高さが調整されるようになりますが、列はコンテナーに基づく固定幅のままになります。

# --hints--

`.image-wrapper` セレクターの `grid-template-columns` プロパティを `2fr 1fr` に設定する必要があります。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.image-wrapper')?.gridTemplateColumns, '2fr 1fr');
```

`.image-wrapper` セレクターの `grid-template-rows` プロパティを `repeat(3, min-content)` に設定する必要があります。
