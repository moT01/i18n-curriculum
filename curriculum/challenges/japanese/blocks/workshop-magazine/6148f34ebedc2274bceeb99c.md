...
雑誌のレイアウトが完了したので、次はレスポンシブ対応にしましょう。

まず `@media` クエリを、`only screen` かつ `max-width` が `720px` の場合を対象にしてください。 その中に `.image-wrapper` セレクターを作成し、`grid-template-columns` プロパティを `1fr` に設定してください。

これで、小さい画面では 3 つの画像が 1 列にまとめられます。

# --hints--

`only screen and (max-width: 720px)` を対象とする新しい `@media` ルールが必要です。

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[0]?.media?.mediaText, 'only screen and (max-width: 720px)');
```

新しい `@media` ルールには `.image-wrapper` セレクターが必要です。

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[0]?.cssRules?.[0]?.selectorText, '.image-wrapper');
```

新しい `.image-wrapper` セレクターの `grid-template-columns` プロパティを `1fr` に設定してください。
