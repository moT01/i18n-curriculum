...
`.publish-date` セレクターを作成し、`color` プロパティを `rgba(255, 255, 255, 0.5)` に設定してください。

# --hints--

`.publish-date` セレクターが必要です。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.publish-date'));
```

`.publish-date` セレクターの `color` プロパティを `rgba(255, 255, 255, 0.5)` に設定する必要があります。
