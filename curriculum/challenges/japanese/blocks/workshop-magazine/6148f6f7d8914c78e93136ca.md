...
最後の 1 つの `@media` クエリを作成し、`only screen` かつ `max-width` が `420px` の場合を対象にしてください。 この中に `.hero-title` セレクターを作成し、`font-size` プロパティを `4.5rem` に設定してください。

おめでとうございます！ 以上で雑誌が完成しました。

# --hints--

`only screen and (max-width: 420px)` を対象とする新しい `@media` クエリが必要です。 このクエリは、一連の `@media` クエリの最後のクエリである必要があります。

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[3]?.media?.mediaText, 'only screen and (max-width: 420px)');
```

新しい `@media` クエリには `.hero-title` セレクターが必要です。

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[3]?.cssRules?.[0]?.selectorText, '.hero-title');
```

`.hero-title` セレクターの `font-size` プロパティを `4.5rem` に設定する必要があります。
