...
もうひとつ `@media` クエリを作成し、`only screen` かつ `max-width` が `600px` の場合を対象にしてください。 その中に `.text-with-images` ルールを作成し、`grid-template-columns` プロパティを `1fr` に設定してください。

これで、更に小さい画面では下部のテキスト領域が 1 列にまとめられます。

# --hints--

`only screen and (max-width: 600px)` を対象とする新しい `@media` ルールが必要です。 このクエリは前の `@media` クエリの下に記述する必要があります。

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[1]?.media?.mediaText, 'only screen and (max-width: 600px)');
```

新しい `@media` クエリには `.text-with-images` セレクターが必要です。

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[1]?.cssRules?.[0]?.selectorText, '.text-with-images');
```

新しい `.text-with-images` セレクターの `grid-template-columns` プロパティを `1fr` に設定する必要があります。
