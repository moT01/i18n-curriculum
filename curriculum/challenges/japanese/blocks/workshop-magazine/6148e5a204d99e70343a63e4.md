...
`gap` プロパティは、`column-gap` と `row-gap` の値を一括指定する方法です。 値を 1 つ指定すると、その値が `column-gap` と `row-gap` の両方に設定されます。 2 つの値を指定した場合は、`row-gap` が最初の値に、`column-gap` が 2 つ目の値に設定されます。

`.image-wrapper` セレクターの `gap` プロパティを `2rem` に設定してください。

# --hints--

`.image-wrapper` 要素の `gap` プロパティを `2rem` に設定する必要があります。
