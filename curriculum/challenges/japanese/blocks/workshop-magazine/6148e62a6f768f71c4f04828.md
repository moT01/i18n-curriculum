...
`place-items` プロパティを使用すると `align-items` と `justify-items` の値を同時に設定できます。 `place-items` プロパティは 1 つまたは 2 つの値を引数に取ります。 値を 1 つ指定した場合は、その値が `align-items` と `justify-items` の両方のプロパティに使用されます。 2 つの値を指定した場合は、最初の値が `align-items` プロパティに使用され、2 つ目の値が `justify-items` プロパティに使用されます。

`.image-wrapper` セレクターの `place-items` プロパティを `center` に設定してください。

# --hints--

`.image-wrapper` セレクターの `place-items` プロパティを `center` に設定する必要があります。
