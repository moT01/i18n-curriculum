...
`.image-1, .image-3` ルールを作成し、その `grid-column` プロパティを `1 / -1` に設定してください。 これで、最初と 3 番目の画像はグリッドの幅全体に広がります。

# --hints--

`.image-1, .image-3` セレクターが必要です。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.image-1, .image-3'));
```

`.image-1, .image-3` セレクターの `grid-column` プロパティを `1 / -1` に設定する必要があります。
