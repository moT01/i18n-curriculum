...
assert.match(shoppingListString, /(?:var|let|const)\s*filteredItems\s*=\s*useMemo\s*\(/);
```
...
assert.match(shoppingListString, /filteredItems\s*=\s*useMemo\s*\(/);
```
...
assert.match(shoppingListString, /filteredItems\s*=\s*useMemo\s*\(\s*(?:function\s*\(\s*\)|\(\s*\)\s*=>)\s*\{\s*return\s+items\.filter\(\s*(?:function\s*\(\s*item\s*\)\s*\{\s*return\s+|\(?\s*item\s*\)?\s*=>\s*)item\.toLowerCase\(\)\.includes\(\s*query\.toLowerCase\(\)\s*\)/);
```
...
assert.match(shoppingListString, /filteredItems\s*=\s*useMemo[\s\S]*,\s*\[\s*query\s*\]\s*\)/);
```
