...
`:root` 内に `--window-color1` という新しい変数を作成し、値を `black` に設定してください。 これは紫色のビルに使うサブカラーになります。

# --hints--

`:root` に `--window-color1` という新しい変数を作成する必要があります。

```js
assert.exists(new __helpers.CSSHelp(document).isPropertyUsed('--window-color1'));
```

カスタムプロパティ (変数) `--window-color1` の値を `black` に設定する必要があります。
