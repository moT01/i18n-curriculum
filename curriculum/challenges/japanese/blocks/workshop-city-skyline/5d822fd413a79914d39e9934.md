...
ついに 最後のビルにたどり着きました！ このビルに `90deg` の方向の繰り返しグラデーションを追加してください。 `0%` から `10%` までビルの色を使用し、`10%` から `30%` まで `transparent` を使用してください。

# --hints--

`.fb6` の `background` プロパティに `repeating-linear-gradient` を追加する必要があります。

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb6")?.background, "repeating-linear-gradient");
```

`repeating-linear-gradient` に `90deg` の方向を指定する必要があります。

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb6")?.getPropVal('background', true), "repeating-linear-gradient(90deg");
```

`repeating-linear-gradient` で最初の色 `--building-color3` を `0%` から `10%` まで設定する必要があります。
...
`repeating-linear-gradient` で 2 番目の色 `transparent` を `10%` から `30%` まで設定する必要があります。
