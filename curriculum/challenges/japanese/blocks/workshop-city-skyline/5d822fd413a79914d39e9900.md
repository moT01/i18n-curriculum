...
次に、 `.bb2a` の 2 つの `#999` を `transparent` に変更してください。 これで、左右の境界線が見えなくなります。

# --hints--

`transparent` を使用するように `border-left` を変更する必要があります 。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb2a")?.borderLeft, "5vw solid transparent");
```

`transparent` を使用するように `border-right` を変更する必要があります 。
