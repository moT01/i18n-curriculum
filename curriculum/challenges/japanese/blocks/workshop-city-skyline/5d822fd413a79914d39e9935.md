...
このビルにもう 1 つ繰り返しグラデーションを追加しましょう。先ほど追加したものと同じ設定にしますが、`90deg` の方向を追加せず、2 つの `transparent` の代わりに窓の色を使用してください。

# --hints--

`.fb6` の `background` プロパティに、2 つ目の `repeating-linear-gradient` を追加する必要があります。
...
2 つ目の `repeating-linear-gradient` で最初の色 `--building-color3` を `0%` から `10%` まで設定する必要があります。
...
2 つ目の `repeating-linear-gradient` で 2 番目の色 `--window-color3` を `10%` から `30%` まで設定する必要があります。
