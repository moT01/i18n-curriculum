...
`background-color` プロパティと値を `.fb3` から削除し、`.fb3a` と `.fb3b` に追加してください。

# --hints--

`.fb3` から `background-color` を削除する必要があります。

```js
assert.isEmpty(new __helpers.CSSHelp(document).getStyle(".fb3")?.backgroundColor);
```

`.fb3a` の `background-color` を `--building-color1` に設定する必要があります。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3a")?.backgroundColor.trim(), "var(--building-color1)");
```

`.fb3b` の `background-color` を `--building-color1` に設定する必要があります。
