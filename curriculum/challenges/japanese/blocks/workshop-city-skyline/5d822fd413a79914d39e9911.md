...
良い感じになってきました！ 手前の建物に取り掛かりましょう！ `.fb1` の中に新しい `div` 要素を 3 つネストして、このビルを 3 つのセクションに分けてください。 それぞれのクラスを順に `fb1a`、`fb1b`、`fb1c` としてください。

# --hints--

`.fb1` 内に `div` 要素を 3 つ追加する必要があります。

```js
assert.lengthOf(document.querySelector("div.fb1")?.children, 3);
```

1 つ目の新しい `div` にクラス `fb1a` を設定する必要があります。

```js
assert.exists(document.querySelector("div.fb1 > div.fb1a"));
```

2 つ目の新しい `div` にクラス `fb1b` を設定する必要があります。

```js
assert.exists(document.querySelector("div.fb1 > div.fb1b"));
```

3 つ目の新しい `div` にクラス `fb1c` を設定する必要があります。

```js
assert.exists(document.querySelector("div.fb1 > div.fb1c"));
```

新しい `div` 要素は `.fb1a + .fb1b + .fb1c`. の順に配置する必要があります。
