...
`bb4-window` クラスの `width` を `18%` に、`height` を `90%` に設定し、`background-color` として `--window-color4` 変数を設定してください。

# --hints--

`.bb4-window` の `width` を `18%` に設定する必要があります。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4-window")?.width, "18%");
```

`.bb4-window` の `height` を `90%` に設定する必要があります。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb4-window")?.height, "90%");
```

`.bb4-window` の `background-color` を `--window-color4` に設定する必要があります。
