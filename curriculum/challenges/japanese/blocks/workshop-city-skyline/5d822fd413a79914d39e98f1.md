...
同じグラデーションを次の 2 つのセクションに追加してみましょう。 その方法として、`bb1-window` という名前の新しいクラスセレクターを作成し、`height` と `background` のプロパティと値を `.bb1a` から新しいクラスセレクターに移動してください。

# --hints--

`bb1-window` という新しいクラスセレクターを作成する必要があります。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.bb1-window'));
```

`height` プロパティと値を `.bb1a` から `.bb1-window` に移動する必要があります。
...
`background` プロパティと値を `.bb1a` から `.bb1-window` に移動する必要があります。
