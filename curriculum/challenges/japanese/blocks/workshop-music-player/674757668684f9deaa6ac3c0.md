...
Still inside the `playSong` function, set the `audio.src` property equal to `song.src`. これにより、選択された曲の音声データがある場所を audio 要素に伝えます。

また、`audio.title` プロパティを `song.title` に等しく設定してください。 これにより、曲のタイトルとして表示する内容をオーディオ要素に伝えます。
