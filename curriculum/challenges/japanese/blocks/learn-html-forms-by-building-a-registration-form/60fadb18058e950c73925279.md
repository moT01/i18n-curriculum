...
`border` を削除して、`2rem` のパディングを各 `fieldset` の上下にだけ追加しましょう。 左右の `padding` を忘れずに削除してください。

# --hints--

`none` または `0` の値を利用して、`border` を削除することができます。

```js
const fieldset = document.querySelector('fieldset');
const computedStyle = getComputedStyle(fieldset);
assert.isTrue(computedStyle.borderStyle == 'none' || computedStyle.borderWidth == '0px')
```

各 `fieldset` の上下に `2rem` の `padding` を追加してください。
...
各 `fieldset` の左右の `padding` を削除してください。
