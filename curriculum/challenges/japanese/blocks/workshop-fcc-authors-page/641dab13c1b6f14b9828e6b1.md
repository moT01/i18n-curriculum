...
あと一歩です。 If you keep clicking the `Load More Authors` button until there's no more data to load and the text changes to `"No more data to load"`, the cursor value is still `pointer`. これも `not-allowed` に変更しましょう。
