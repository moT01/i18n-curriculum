...
assert.match(
  ...
  __helpers.concatRegex(
    /sumSquaredDifferences\s*=\s*squaredDifferences\.reduce\(\s*/,
    __helpers.functionRegex(null, ['acc', 'el'], { includeBody: false })
  )
);
```
...
```js
assert.match(getVariance.toString(), /sumSquaredDifferences\s*=\s*squaredDifferences\.reduce\([\s\S]*,\s*0\s*\)/);
```
