...
```js
  try {
    ...
    if (res.ok) {
      ...
      await waitFor(() => creatureName.innerText.trim());
...
```js
  try {
    ...
    if (res.ok) {
      ...
      await waitFor(() => typesEl.children.length === 1);
...
```js
  try {
    ...
    if (res.ok) {
      ...
      const typesEl = document.getElementById('types');
      await waitFor(() => typesEl.children.length === targetTypes.length);
...
```js
  try {
    ...
    if (res.ok) {
      ...
      await waitFor(() => typesEl.children.length === actualTypes.length);
...
```js
  ...
  searchInput.dispatchEvent(new Event('change'));
  searchButton.click();
  await waitFor(() => spy.calls.length >= 1);

  searchInput.value = '2';
  searchInput.dispatchEvent(new Event('change'));
  searchButton.click();
  await waitFor(() => spy.calls.length >= 2);
  ...
  searchInput.dispatchEvent(new Event('change'));
  searchButton.click();
  await waitFor(() => spy.calls.length >= 3);
