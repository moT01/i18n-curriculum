...
- The **`slice()` Method**: This method returns a new string containing a portion of the original string, determined by the specified start and end indices. It does not modify the original string. The start index is included, while the end index is excluded.
