...
`.info` 要素内のすべての `label` 要素を選択し、`width` を `10%` に設定し、さらに、`55px` より小さくならないようにしてください。

# --hints--

`.info label` または `.info > label` のいずれかのセレクターを使用する必要があります。
...
`label` 要素の `width` を `10%` に設定する必要があります。
...
`label` 要素の `min-width` を `55px` に設定する必要があります。
