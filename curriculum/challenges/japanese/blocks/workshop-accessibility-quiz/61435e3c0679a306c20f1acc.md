...
ページ内を移動できるようにするには、各アンカー要素に `h2` 要素の `id` に対応する `href` を設定してください。

# --hints--

1 つ目の `a` 要素には値が `#student-info` に設定されている `href` が必要です。

```js
assert.equal(document.querySelectorAll('a')?.[0]?.getAttribute('href'), '#student-info');
```

2 つ目の `a` 要素には値が `#html-questions` に設定されている `href` が必要です。

```js
assert.equal(document.querySelectorAll('a')?.[1]?.getAttribute('href'), '#html-questions');
```

3 つ目の `a` 要素には値が `#css-questions` に設定されている `href` が必要です。
