...
`textarea` 要素を 2 つ目の `div.answer` 要素内にネストし、行数と幅を設定してください。

# --hints--

2 つ目の `div.answer` 要素内に、`textarea` 要素を 1 つネストする必要があります。

```js
assert.exists(document.querySelectorAll('div.answer')?.[1]?.querySelector('textarea'));
```

`textarea` に `rows` 属性を追加し、数値を設定する必要があります。
...
`textarea` に `cols` 属性を追加し、数値を設定する必要があります。
