...
1 つ目の `h2` 要素の `id` 属性の値を `student-info` に設定する必要があります。

```js
assert.equal(document.querySelectorAll('h2')?.[0]?.id, 'student-info');
```

2 つ目の `h2` 要素の `id` 属性の値を `html-questions` に設定する必要があります。
...
1 つ目の `h2` 要素に適切なテキストコンテンツを入れる必要があります。 _ヒント: 例えば `Student Info` などが適切でしょう。_

```js
assert.isNotEmpty(document.querySelectorAll('h2')[0]?.textContent.trim());
```

2 つ目の `h2` 要素に適切なテキストコンテンツを入れる必要があります。 _ヒント: 例えば `HTML` などが適切でしょう。_

```js
assert.isNotEmpty(document.querySelectorAll('h2')[1]?.textContent.trim());
```

3 つ目の `h2` 要素に適切なテキストコンテンツを入れる必要があります。 _ヒント: 例えば `CSS` などが適切でしょう。_
