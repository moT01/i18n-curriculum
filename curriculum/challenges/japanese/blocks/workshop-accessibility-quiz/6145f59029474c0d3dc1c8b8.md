...
`input` と `label` 要素の場合と同様に、`textarea` を対応する `label` 要素に関連付け、さらに `name` 属性を加えてください。

# --hints--

`textarea` 要素に `id` 属性が必要です。

```js
assert.notEmpty(document.querySelectorAll('.answer')?.[1]?.querySelector('textarea')?.id);
```

`textarea` 要素の `id` にスペースを含めることはできません。

```js
assert.match(document.querySelectorAll('.answer')?.[1]?.querySelector('textarea')?.id, /^\S+$/);
```

別の要素が同じ `id` を使用しているため、`textarea` 要素に対して別の `id` を選択する必要があります。
...
`label` 要素の `for` 属性を、`textarea` 要素の `id` と一致するように設定してください。
