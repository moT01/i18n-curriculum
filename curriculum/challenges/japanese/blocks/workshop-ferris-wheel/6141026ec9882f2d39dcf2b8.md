...
`0%` と `100%` のセレクターの間に、`50%` セレクターを追加してください。 これはアニメーションサイクルの中間に適用されます。 その `background-color` を `purple` に設定してください。

# --hints--

新しく `50%` セレクターを `@keyframes cabins` ルール内に作成する必要があります。
