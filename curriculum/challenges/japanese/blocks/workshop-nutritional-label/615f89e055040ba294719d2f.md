...
太い仕切り線の下に `p` をもう 1 つ作成してください。 `p` 要素のテキストは `Vitamin D 2mcg 10%` としてください。

今、`p` 要素にはテキストだけが含まれている状態です。ではパーセンテージの部分を `span` 要素で囲んで、残りのテキストと分けて右揃えにしましょう。
...
新しい `p` 要素は `Vitamin D 2mcg 10%` というテキストを持つ必要があります。
