...
プレビューで、テキストの `8g` の部分が中央に表示されていることに注目してください。 この配置を調整するため、`Total Fat` を囲んでいる `span` 要素とテキストの `8g` の部分をまとめるように、外側にもう 1 つ `span` 要素を追加してください。

# --hints--

コードの `<span class="bold">Total Fat</span> 8g` という部分の周りを、もう 1 つ別の `span` 要素で囲んでください。
