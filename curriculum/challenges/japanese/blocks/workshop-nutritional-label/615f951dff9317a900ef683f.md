...
`.note` セレクターの左右のパディングを `8px` に設定し、上下のパディングを削除してください。 また、`text-indent` プロパティを `-8px` に設定してください。

この最後の変更で、栄養成分表示ラベルの完成です！

# --hints--

`.note` セレクターの `padding` プロパティを `0 8px` に設定する必要があります。
...
`.note` セレクターの `text-indent` プロパティを `-8px` に設定する必要があります。
