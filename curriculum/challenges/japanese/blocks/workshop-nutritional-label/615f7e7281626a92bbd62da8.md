...
最後の `p` 要素の下に `p` 要素をもう 1 つ作成し、テキストを `Sodium 160mg 7%` にしてください。 前のステップと同様に、`Sodium` の部分と `7%` の部分をそれぞれ `span` 要素で囲み、両方にクラス `bold` を設定してください。

そして、もう 1 つの `span` 要素で `Sodium 160mg` の部分を囲み、正しく配置されるようにしてください。
