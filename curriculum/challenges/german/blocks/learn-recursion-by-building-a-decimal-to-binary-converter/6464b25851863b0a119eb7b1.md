...
Your `showAnimation()` function should have three `setTimeout()` functions.
...
Die neue `setTimeout()`-Funktion sollte eine leere Callback-Funktion als erstes Argument haben.

```js
assert.match(
  String(showAnimation),
  /setTimeout\s*\(\s*(?:function\s*\(\s*\)\s*{\s*|\(\s*\)\s*=>\s*{\s*)/
);
```
...
assert.match(
  String(showAnimation),
  /setTimeout\s*\([\s\S]*removeElDelay[\s\S]*\)/
);
```
