...
assert.match(shoppingListString, /React\.createElement\("ul",\s*null,\s*filteredItems\.map\(/);
```
...
assert.match(shoppingListString, /filteredItems\.map\(\s*(?:function\s*\(\s*item\s*\)|\(?\s*item\s*\)?\s*=>)/);
```
...
assert.match(shoppingListString, /filteredItems\.map\([\s\S]*React\.createElement\("li",[\s\S]*item\)/);
```
