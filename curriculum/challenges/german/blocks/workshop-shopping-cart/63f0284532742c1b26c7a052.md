...
Füge der `forEach`-Methode deine Callback-Funktion hinzu. Sie sollte einen `btn`-Parameter verwenden. Füge dann im Callback einen Ereignis-Listener zu `btn` hinzu. Der Ereignis-Listener sollte auf ein `click`-Ereignis warten und einen weiteren Callback mit einem `event`-Parameter annehmen. Dein zweites Callback sollte leer sein.

# --hints--

Du solltest die Arrow-Syntax verwenden, um der `forEach`-Methode eine Callback-Funktion hinzuzufügen, die einen `btn`-Parameter verwendet.

```js
assert.match(code, /\[\s*\.\.\.addToCartBtns\s*\]\s*\.\s*forEach\s*\(\s*(\(\s*btn\s*\)|btn)\s*=>\s*(?:\{|\s*)/);
```

Du solltest einen Event-Listener zu deiner `btn`-Variablen hinzufügen.
...
Du solltest auf ein `click`-Ereignis für die `btn`-Variable achten.
...
Du solltest dem Event-Listener eine leere Callback-Funktion hinzufügen. Denke daran, ihr einen `event`-Parameter zuzuweisen.
