...
Auch bei dieser Methode wirst du das HTML aktualisieren. Setze den `textContent` der `cartSubTotal` auf den Wert von `subTotal` mit 2 festen Dezimalstellen. Verwende die Template-Literal-Syntax, um das Dollarzeichen an den Anfang des Wertes zu setzen.

# --hints--

Du solltest auf die `textContent`-Eigenschaft des `cartSubTotal`-Elements zugreifen.

```js
assert.match(cart.calculateTotal.toString(), /cartSubTotal\.textContent/);
```

Du solltest die `.toFixed()`-Methode für die `subTotal`-Variable aufrufen und `2` als Argument übergeben.

```js
assert.match(cart.calculateTotal.toString(), /subTotal\.toFixed\(\s*2\s*\)/);
```

Du solltest die Template-Literal-Syntax verwenden, um das Dollarzeichen vor deinem `.toFixed()`-Aufruf hinzuzufügen.
...
Du solltest den `textContent` des `cartSubTotal`-Elements auf deinen Template-String setzen.
