...
Füge innerhalb deines `div` zwei `p`-Elemente hinzu. Setze den Text deines zweiten `p`-Elements auf den Wert der `price`-Variable.

# --hints--

Du solltest innerhalb deines `div`-Elements zwei `p`-Elemente hinzufügen.
...
Dein zweites `p`-Element sollte den Text der `price`-Variable haben.
