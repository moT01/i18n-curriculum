...
Deine `ShoppingCart`-Klasse sollte in der Lage sein, Artikel hinzuzufügen. Erstelle eine leere `addItem`-Methode, die zwei Parameter benötigt: `id` und `products`. So könnte die Erstellung einer Methode aussehen:
...
Der erste Parameter, `id`, ist die `id` des Produkts, das der Nutzer in seinen Einkaufswagen gelegt hat. The second parameter, `products`, is an array of `Dessert` instances. Die Verwendung eines Parameters anstelle eines direkten Verweises auf dein bestehendes `products`-Array macht diese Methode flexibler, wenn du zukünftig weitere Produktlisten hinzufügen möchtest.

# --hints--

Deine `ShoppingCart`-Klasse sollte über eine `addItem`-Methode verfügen.
...
Deine `addItem`-Methode sollte zwei Parameter verwenden: `id` und `products`.
...
Deine `addItem`-Methode sollte leer sein.
