...
Du musst die verfügbaren Produkte in deinem HTML anzeigen. Beginne damit, den Additions-Zuweisungsoperator zu verwenden, um der `innerHTML`-Eigenschaft der `dessertCards`-Variablen einen leeren Template-Literal-String hinzuzufügen.
...
Du solltest den Additions-Zuweisungsoperator für die `innerHTML`-Eigenschaft verwenden.

```js
assert.match(code, /dessertCards\.innerHTML\s*\+=\s*/);
```

Du solltest einen leeren Template-Literal-String zur `innerHTML`-Eigenschaft hinzufügen.
