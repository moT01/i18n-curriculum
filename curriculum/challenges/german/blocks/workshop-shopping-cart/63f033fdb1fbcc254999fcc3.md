...
Dein Einkaufswagen ist derzeit nicht auf der Website sichtbar. Um ihn sichtbar zu machen, füge einen Ereignis-Listener zu der `cartBtn`-Variablen hinzu, und höre auf das Klick-Ereignis. Der Callback benötigt keine Parameter.

# --hints--

Du solltest einen Event-Listener zu deiner `cartBtn`-Variablen hinzufügen.

```js
assert.match(code, /cartBtn\s*\.\s*addEventListener\s*\(/);
```

Du solltest auf ein `click`-Ereignis für die `cartBtn`-Variable achten.

```js
assert.match(code, /cartBtn\s*\.\s*addEventListener\s*\(\s*('|"|`)click\1\s*/);
```

Du solltest eine leere Callback-Funktion (mit der Arrow-Syntax) zu dem Event-Listener hinzufügen. Denke daran, dass sie keine Parameter benötigt.
