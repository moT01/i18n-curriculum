...
Es gibt immer noch mehr Funktionen, die deine `ShoppingCart`-Klasse benötigt, aber zuerst musst du in der Lage sein, den Code zu testen, den du gerade geschrieben hast. Du musst ein neues `ShoppingCart`-Objekt <dfn>instantiieren</dfn> und es einer Variablen zuweisen. Hier ist ein Beispiel für die Instanziierung der `Computer`-Klasse aus früheren Beispielen:

```js
const myComputer = new Computer();
```

Deklariere eine `cart`-Variable, und weise ihr ein neues `ShoppingCart`-Objekt zu. Man beachte die Verwendung des `new`-Schlüsselworts bei der Instanziierung des Objekts.

# --hints--

Du solltest `const` verwenden, um eine `cart`-Variable zu deklarieren.
...
Du solltest dein neues `ShoppingCart`-Objekt deiner `cart`-Variablen zuweisen.
