...
Erstelle zwei `p`-Elemente nach dem `h2`-Element. Give the first a `class` of `dessert-price`, and set the text to a dollar sign `"$"` followed by the value of the `price` variable. Give the second a `class` of `product-category`, and the text `"Category: "` followed by the value of the `category` variable.

# --hints--

Du solltest zwei `p`-Elemente erstellen.
...
Dein `p`-Element sollte nach deinem `h2`-Element stehen.
...
Dein erstes `p`-Element sollte eine `class` auf `dessert-price` gesetzt haben.
