...
Du solltest deinen Code optimieren. Verschiebe die `position` und `top` Eigenschaften und Werte von `.foreground-buildings` zu `.background-buildings`. Wähle dort dann beide `.background-buildings` und `.foreground-buildings`, wobei du diese Styles effektiv auf beide Elemente anwendest. Du kannst ein Komma (`,`) verwenden, um die Selektoren wie folgt zu trennen: `selector1, selector2`.
