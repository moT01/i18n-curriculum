...
Du solltest drei `div`-Elemente innerhalb von `.fb1` hinzufügen.

```js
assert.lengthOf(document.querySelector("div.fb1")?.children, 3);
```

Du solltest dem ersten neuen `div` eine Klasse von `fb1a` zuweisen.

```js
assert.exists(document.querySelector("div.fb1 > div.fb1a"));
```

Du solltest dem zweiten neuen `div` eine Klasse von `fb1b` geben.

```js
assert.exists(document.querySelector("div.fb1 > div.fb1b"));
```

Du solltest dem dritten neuen `div` eine Klasse von `fb1c` zuweisen.

```js
assert.exists(document.querySelector("div.fb1 > div.fb1c"));
```

Du solltest die neuen `div`-Elemente in dieser Reihenfolge platzieren: `.fb1a + .fb1b + .fb1c`.
