...
Endlich! Du hast es bis zum letzten Gebäude geschafft! Füge ihm einen wiederholenden Farbverlauf mit einer `90deg` Richtung hinzu. Verwende die Gebäudefarbe von `0%` zu `10%` und `transparent` from `10%` zu `30%`.

# --hints--

Du solltest in der `background`-Eigenschaft einen `repeating-linear-gradient` zu `.fb6` hinzufügen.
