...
Ändere als Nächstes die beiden `#999` von `.bb2a` zu `transparent`. Dadurch wird der linke und rechte Rand unsichtbar.

# --hints--

Du solltest den `border-left` so ändern, dass er `transparent` verwendet.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".bb2a")?.borderLeft, "5vw solid transparent");
```

Du solltest den `border-right` so ändern, dass er `transparent` verwendet.
