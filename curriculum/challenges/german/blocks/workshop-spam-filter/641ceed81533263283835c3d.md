...
Statt der `.match()`-Methode kannst du auch die `.test()`-Methode eines regulären Ausdrucks verwenden, um zu prüfen, ob ein String dem Muster entspricht. Im Gegensatz zu `.match()` gibt `.test()` ein booleschen Wert zurück, der angibt, ob der String dem Muster entspricht oder nicht.

Update your `isSpam()` function to use the `.test()` method on `helpRegex` to test if `msg` is a match.

Gib dann einige Nachrichten auf deiner Seite ein und sieh dir das Ergebnis an.
