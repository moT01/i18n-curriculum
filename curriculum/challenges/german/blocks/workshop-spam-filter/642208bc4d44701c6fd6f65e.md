...
Der Dollarwert kann mehr als eine Ziffer betragen. To match this, the `+` quantifier can be used - this matches one or more consecutive occurrences. Der reguläre `/a+/`-Ausdruck stimmt beispielsweise mit einem oder mehreren aufeinanderfolgenden `a`-Zeichen überein.

Aktualisiere deinen regulären Ausdruck auf eine oder mehrere aufeinanderfolgende Ziffern.

# --hints--

Deine `dollarRegex` sollte den `+`-Quantifizierer verwenden.

```js
assert.match(dollarRegex.source, /\+/);
```

Deine `dollarRegex` sollte den `+`-Quantifizierer auf deiner `[0-9]`-Zeichenklasse verwenden.
