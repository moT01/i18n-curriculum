...
For the condition of the `if` statement, check if the `target` is not an instance of `HTMLElement`. Then inside it, add a `return`.

# --hints--

You should check in your `if` statement should have the condition `!(target instanceof  HTMLElement)`.

```js
assert.match(__helpers.removeJSComments(code), /if\s*\(\s*!\s*\(\s*target\s+instanceof\s+HTMLElement\s*\)\s*\)/);
```

You should have a `return` inside your `if` statement.

```js
assert.match(__helpers.removeJSComments(code), /if\s*\(\s*!\s*\(\s*target\s+instanceof\s+HTMLElement\s*\)\s*\)\s*\{\s*return\s*/);
```
