...
Check if `cardElement` is not an instance of `HTMLElement`, and if so, add a `return`

# --hints--

You should add another `if` statement with the condition `!(cardElement instanceof HTMLElement)`.

```js
assert.match(__helpers.removeJSComments(code), /if\s*\(\s*!\s*\(\s*cardElement\s+instanceof\s+HTMLElement\s*\)\s*\)/);
```

You should have a `return` inside your `if` statement.

```js
assert.match(__helpers.removeJSComments(code), /if\s*\(\s*!\s*\(\s*cardElement\s+instanceof\s+HTMLElement\s*\)\s*\)\s*\{\s*return\s*/);
```
