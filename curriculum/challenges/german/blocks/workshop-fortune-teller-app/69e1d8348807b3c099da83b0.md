...
You should test your progress so far. Create an instance of the `Game` class and assign it to a `game` variable.

At this point, you'll see that you can see the cards when you click the Single Card Reading and Three Card Spread buttons.

# --hints--

You should declare a variable named `game`.

```js
const explorer = await __helpers.Explorer(code);
assert.exists(explorer.variables.game);
```

Your `game` variable should be initialized with `new Game()`.

```js
const explorer = await __helpers.Explorer(code);
assert.isTrue(explorer.variables.game.value.matches("new Game()"));
```
