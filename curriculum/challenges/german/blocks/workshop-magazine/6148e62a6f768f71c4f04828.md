...
Die `place-items`-Eigenschaft kann verwendet werden, um die `align-items` und `justify-items`-Werte gleichzeitig festzulegen. Die `place-items`-Eigenschaft nimmt ein oder zwei Werte ein. Wenn ein Wert angegeben wird, wird er für die `align-items` und `justify-items`-Eigenschaften verwendet. Wenn zwei Werte gegeben werden, wird der erste Wert für die `align-items`-Eigenschaft und der zweite Wert für die `justify-items`-Eigenschaft verwendet.

Gib dem `.image-wrapper`-Selektor eine `place-items`-Eigenschaft mit dem Wert `center`.

# --hints--

Dein `.image-wrapper`-Selektor sollte eine `place-items`-Eigenschaft auf `center` gesetzt haben.
