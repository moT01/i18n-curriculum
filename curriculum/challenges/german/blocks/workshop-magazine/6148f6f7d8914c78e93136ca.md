...
Erstelle eine letzte `@media`-Query für `only screen` mit einer `max-width` von `420px`. Erstelle innerhalb der Media Query einen `.hero-title`-Selektor mit einer `font-size`-Eigenschaft mit dem Wert `4.5rem`.

Glückwunsch! Dein Magazin ist nun fertig.
...
Deine neue `@media`-Query sollte einen `.hero-title`-Selektor enthalten.

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[3]?.cssRules?.[0]?.selectorText, '.hero-title');
```

Dein `.hero-title`-Selektor sollte eine `font-size`-Eigenschaft mit dem Wert `4.5rem` enthalten.
