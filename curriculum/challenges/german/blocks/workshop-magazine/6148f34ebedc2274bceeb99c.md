...
Da das Layout des Magazins fertig ist, musst du es responsive gestalten.

Beginne mit einer `@media`-Query für `only screen` mit einer `max-width` von `720px`. Erstelle innen einen `.image-wrapper`-Selektor und weise ihm eine `grid-template-columns`-Eigenschaft mit dem Wert `1fr` zu.
