...
Erstelle unterhalb deines `.first-paragraph`-Elements ein weiteres `p`-Element und weise ihm folgenden Text zu:
...
Du solltest ein zweites `p`-Element innerhalb deines `.text`-Elements erstellen.

```js
assert.lengthOf(document.querySelectorAll('.text p'), 2);
```

Dein zweites `p`-Element sollte den bereitgestellten Text enthalten.
