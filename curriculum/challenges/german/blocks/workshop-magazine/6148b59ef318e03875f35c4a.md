...
Erstelle einen `img` Selektor und gib ihm eine `width` Eigenschaft von `100%`, und eine `object-fit` Eigenschaft von `cover`.

Die `object-fit`-Eigenschaft zeigt dem Browser, wie das Element innerhalb seines Containers positioniert werden soll. In diesem Fall wird `cover` das Bild so einstellen, dass es den Container füllt, indem es nach Bedarf zuschneidet, um das Seitenverhältnis nicht zu verändern.
...
Dein `img`-Selektor sollte eine `width`-Eigenschaft mit dem Wert `100%` enthalten.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('img')?.width, '100%');
```

Dein `img`-Selektor sollte eine `object-fit`-Eigenschaft mit dem Wert `cover` enthalten.
