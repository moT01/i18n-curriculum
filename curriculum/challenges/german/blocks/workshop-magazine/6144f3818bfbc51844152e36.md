...
Verwende die `minmax`-Funktion, um deine Spalten auf jedem Gerät responsiv zu machen. Die `minmax`-Funktion nimmt zwei Argumente, zum einen den Mindestwert und zum anderen das Maximum. Diese Werte können eine Länge, Prozentsatz, `fr` oder sogar ein Schlüsselwort wie `max-content` sein.

Umbreche jedes deiner bereits definierten Werte der `grid-template-columns`-Eigenschaft in einer `minmax` Funktion, indem jeder Wert als zweites Argument verwendet wird. Das erste Argument sollte `2rem`, `min-content` und beziehungsweise `2rem` sein.

# --hints--

Dein `main`-Selektor sollte eine `grid-template-columns`-Eigenschaft von `minmax(2rem, 1fr) minmax(min-content, 94rem) minmax(2rem, 1fr)` haben.
