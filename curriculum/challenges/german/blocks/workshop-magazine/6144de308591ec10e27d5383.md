...
Erstelle einen `h1`-Selektor und setze die `font-family`-Eigenschaft auf `Anton` mit `sans-serif` als Fallback-Wert.
...
Dein `h1`-Selektor sollte eine `font-family`-Eigenschaft mit dem Wert `Anton` zusammen mit `sans-serif` als Fallback-Wert enthalten.
