...
Die Bilder sollten sich innerhalb eines zweispaltigen, drei-Zeilen-Layouts befinden.
...
Dein `.image-wrapper`-Selektor sollte eine `grid-template-columns`-Eigenschaft mit dem Wert `2fr 1fr` haben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.image-wrapper')?.gridTemplateColumns, '2fr 1fr');
```

Dein `.image-wrapper`-Selektor sollte eine `grid-template-rows`-Eigenschaft mit dem Wert `repeat(3, min-content)` haben.
