...
Erstelle am Ende deines `.text`-Elements ein letztes `p`-Element und gib ihm den folgenden Text:
...
Du solltest ein sechstes `p`-Element zu dem `.text`-Element hinzufügen.

```js
assert.lengthOf(document.querySelectorAll('.text p'), 7);
```

Dein sechstes `p` sollte den bereitgestellten Text enthalten.
