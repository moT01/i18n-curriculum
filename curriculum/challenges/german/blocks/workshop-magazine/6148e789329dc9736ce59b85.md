...
Erstelle eine `.image-1, .image-3`-Regel und gib ihr eine `grid-column`-Eigenschaft mit dem Wert `1 / -1`. Dadurch können die ersten und dritten Bilder die volle Breite des Gitters überschreiten.

# --hints--

Du solltest einen `.image-1, .image-3`-Selektor haben.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.image-1, .image-3'));
```

Dein `.image-1, .image-3`-Selektor sollte eine `grid-column`-Eigenschaft auf `1 / -1` gesetzt haben.
