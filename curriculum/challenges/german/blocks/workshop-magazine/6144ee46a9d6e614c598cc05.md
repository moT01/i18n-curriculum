...
Erstelle einen `a` Selektor und gib ihm eine `text-decoration` Eigenschaft von `none`, und eine `color` Eigenschaft von `linen`.

# --hints--

Du solltest einen `a`-Selektor haben.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('a'));
```

Dein `a`-Selektor sollte eine `text-decoration`-Eigenschaft mit `none` haben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('a')?.textDecoration, 'none');
```

Dein `a`-Selektor sollte eine `color`-Eigenschaft mit `linen` haben.
