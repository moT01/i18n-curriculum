...
Gib dem `.text`-Selektor eine `font-size` Eigenschaft mit dem Wert `1.8rem` und eine `letter-spacing`-Eigenschaft mit dem Wert `0.6px`.

# --hints--

Dein `.text`-Selektor sollte eine `font-size`-Eigenschaft mit dem Wert `1.8rem` haben.

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.text')?.fontSize, '1.8rem');
```

Dein `.text`-Selektor sollte eine `letter-spacing`-Eigenschaft mit dem Wert `0.6px` haben.
