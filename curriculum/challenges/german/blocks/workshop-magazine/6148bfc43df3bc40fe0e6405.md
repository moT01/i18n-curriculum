...
Erstelle einen `.publish-date`-Selektor und gib ihm eine `color`-Eigenschaft mit dem Wert `rgba(255, 255, 255, 0.5)`.

# --hints--

Du solltest einen `.publish-date`-Selektor haben.

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.publish-date'));
```

Dein `.publish-date`-Selektor sollte eine `color`-Eigenschaft mit dem Wert `rgba(255, 255, 255, 0.5)` haben.
