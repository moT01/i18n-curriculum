...
Füge innerhalb des `tbody`-Elements vier `tr`-Elemente hinzu. Gib den ersten drei das `class`-Attribut von `data` und dem vierten das `class`-Attribut von `total`.

# --hints--

Dein `tbody`-Element sollte vier `tr`-Elemente haben.
...
Deine ersten drei `tr`-Elemente sollten das `class` auf `data` gesetzt haben.
