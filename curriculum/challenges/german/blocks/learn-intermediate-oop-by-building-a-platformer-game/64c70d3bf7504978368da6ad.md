...
Du solltest eine `movePlayer`-Funktion aufrufen.
...
Deine `movePlayer`-Funktion sollte leer sein.

```js
assert.match(movePlayer.toString(), /\(\s*key\s*,\s*xVelocity\s*,\s*isPressed\s*\)\s*=>\s*\{\s*\}/);
```
