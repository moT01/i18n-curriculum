...
Du solltest dem ersten `a`-Element ein `href` von `#student-info` geben.

```js
assert.equal(document.querySelectorAll('a')?.[0]?.getAttribute('href'), '#student-info');
```

Du solltest dem zweiten `a`-Element ein `href` von `#html-questions` geben.

```js
assert.equal(document.querySelectorAll('a')?.[1]?.getAttribute('href'), '#html-questions');
```

Du solltest dem dritten `a`-Element ein `href` von `#css-questions` geben.
