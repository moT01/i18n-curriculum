...
Erfasse alle `label`-Elemente innerhalb der `.info`-Elemente und setze ihre `width` auf `10%` – so, dass sie nicht weniger als `55px` in Anspruch nehmen.

# --hints--

Du solltest entweder den `.info label`- oder den `.info > label`-Selektor verwenden.
...
Du solltest den `label`-Elementen eine `width` von `10%` geben.
...
Du solltest den `label`-Elementen eine `min-width` von `55px` geben.
