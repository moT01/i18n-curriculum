...
Um die Eingabefelder aneinander auszurichten, erstelle einen neuen Regelsatz, der auf alle `input`- und `label`-Elemente innerhalb eines `.info`-Elements abzielt und setze die `display`-Eigenschaft auf `inline-block`.

Richte außerdem den Text des `label`-Elements nach rechts aus.

# --hints--

Du solltest entweder den `.info > label, .info > input`- oder `.info label, .info input`-Selektor verwenden.
...
Du solltest den `input`- und `label`-Elementen ein `display` von `inline-block` geben.
...
Du solltest dem `label`-Element ein `text-align` von `right` geben.
