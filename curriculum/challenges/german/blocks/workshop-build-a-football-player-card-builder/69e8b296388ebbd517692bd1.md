...
Your should have a `div` element with the `className` of `form-group`.

```js
const container = await __helpers.prepTestComponent(window.index.FootballPlayerCard);
const formGroupDiv = container.querySelectorAll('.form-panel > div > div.form-group');
assert.lengthOf(formGroupDiv, 3);
```

Your `div.form-group` should have a `label` with an `htmlFor` attribute of `imageUrl`, a `className` of `label`, and the text `Image URL`.

```js
const container = await __helpers.prepTestComponent(window.index.FootballPlayerCard);
const label = container.querySelector('label.label[for="imageUrl"]');
assert.exists(label);
assert.include(label.textContent.trim(), 'Image URL');
```

Your Image URL `form-group` should contain an `input` with an `id` of `imageUrl`, `className` of `input`, and a `type` of `text`.

```js
const container = await __helpers.prepTestComponent(window.index.FootballPlayerCard);
assert.exists(container.querySelector('input#imageUrl.input[type="text"]'));
```
