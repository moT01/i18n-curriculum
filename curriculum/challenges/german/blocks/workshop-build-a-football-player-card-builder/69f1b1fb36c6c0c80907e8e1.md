...
Your `div.preview-panel` element should contain a `p` with the `className` of `preview-label` and the text `Live Preview`.

```js
const container = await __helpers.prepTestComponent(window.index.FootballPlayerCard);
const el = container.querySelector('.preview-panel p.preview-label');
assert.exists(el);
assert.strictEqual(el.textContent.trim(), 'Live Preview');
```

Your `div.preview-panel` should contain a `p` with the `className` of `preview-hint` and the text `Updates as you type`.

```js
const container = await __helpers.prepTestComponent(window.index.FootballPlayerCard);
const el = container.querySelector('.preview-panel p.preview-hint');
assert.exists(el);
assert.strictEqual(el.textContent.trim(), 'Updates as you type');
```

Your `div.preview-panel` should contain a `div` with the `className` of `preview-box` and a dynamic tier `className` prefixed with `tier-`.

```js
const container = await __helpers.prepTestComponent(window.index.FootballPlayerCard);
const el = container.querySelector('.preview-panel div.preview-box');
assert.exists(el);
...
```js
const container = await __helpers.prepTestComponent(window.index.FootballPlayerCard);
assert.exists(container.querySelector('.preview-box .card'));
```
