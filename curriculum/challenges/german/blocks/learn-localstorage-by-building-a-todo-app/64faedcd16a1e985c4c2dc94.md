...
Du solltest `const` und Arrow-Syntax verwenden, um eine `deleteTask`-Funktion zu erstellen.

```js
assert.match(code, /const\s+deleteTask\s*=\s*(\(.*\)|[^\s()]+)\s*=>\s*/);
```

Deine `deleteTask`-Funktion sollte einen `buttonEl`-Parameter verwenden.

```js
assert.match(deleteTask.toString(), /\(?\s*buttonEl\s*\)?/);
```

Deine `deleteTask`-Funktion sollte leer sein.
