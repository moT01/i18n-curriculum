...
Eine Sache noch. If you keep clicking the `Load More Authors` button until there's no more data to load and the text changes to `"No more data to load"`, the cursor value is still `pointer`. Warum nicht stattdessen den Cursorwert in `not-allowed` ändern?
...
Du solltest auf die `style`-Eigenschaft von `loadMoreBtn` mit einer Punktnotation zugreifen.

```js
assert.match(code, /loadMoreBtn\.style/)
```

Du solltest den `cursor` aus der `style`-Eigenschaft mit der Punktschreibweise aufrufen.
