...
Füge einen neuen `#piano`-Selektor innerhalb deiner `@media` Query hinzu und setze die `width` auf `358px`.

# --hints--

Deine `@media`-Regel sollte einen `#piano`-Selektor enthalten.
...
Dein neuer `#piano`-Selektor sollte eine `width` von `358px` enthalten.
