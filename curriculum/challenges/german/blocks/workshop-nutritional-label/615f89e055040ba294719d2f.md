...
Erstelle ein weiteres `p`-Element unter deinem großen Trenner. Weise dem `p`-Element den Text `Vitamin D 2mcg 10%` zu.

Das `p`-Element enthält nur Text. Du kannst den Prozentsatz in einem `span`-Element einbetten, so dass es als eine vom übrigen Text getrennte Einheit betrachtet und nach rechts verschoben wird.
...
Dein neues `p`-Element sollte den Text `Vitamin D 2mcg 10%` enthalten.
