...
Füge unter deinem `p`-Element ein weiteres `p`-Element mit dem Text `Total Carbohydrate 37g 13%` hinzu. Verwende wie zuvor `span`-Elemente, um den Text `Total Carbohydrate` und `13%` fettgedruckt zu machen. Dann stecke Nährstoff und Menge in ein `span`-Element um wiederum die Ausrichtung zu gestalten.
...
Dein neues `p`-Element sollte den Text `Total Carbohydrate 37g 13%` haben.
...
Ein `span`-Element sollte den Text `Total Carbohydrate 37g` einschließen.
...
Ein `span`-Element sollte den Text `Total Carbohydrate` umschließen.
...
Das `span`-Element um `Total Carbohydrate` sollte das `class`-Attribut auf `bold` gesetzt haben.
