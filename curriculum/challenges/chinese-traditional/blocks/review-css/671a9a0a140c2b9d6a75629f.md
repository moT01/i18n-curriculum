...
- **什麼是 CSS？**：層疊樣式表（CSS）是一種標記語言，用於在 HTML 元素上應用樣式。 CSS 用於顏色、背景圖像、佈局等。
...
- **塊級元素**：默認情況下，塊級元素佔據其可用的全部寬度，橫跨其容器的寬度。 一些常見的塊級元素有 `div`、`paragraph` 和 `section` 元素。
...
- **通用選擇器（`*`）**：是一種特殊的 CSS 選擇器，可匹配文檔中的任何元素。 它通常用於將一種樣式應用於頁面上的所有元素，這對於在不同瀏覽器中重置或規範樣式非常有用。 在所有選擇器中，通用選擇器的優先級值最低。 它對優先級值的所有部分 (0, 0, 0, 0) 的貢獻均爲 0。
...
- **留白（負空間）**：設計中的空白空間。 這是元素周圍的區域。
- **UI（用戶界面）**：UI 包括用戶可以在屏幕上看到的視覺和交互元素，如圖標、圖像、文本、菜單、鏈接和按鈕。
- **UX（用戶體驗）**：UX 是指用戶在使用產品或服務時的感受。 一個具有精心設計的用戶體驗的應用程序是直觀的、易於使用的、高效的、可訪問的和令人愉悅的。
...
- **`:in-range` 和 `:out-of-range` 僞類**：這些僞類根據元素值是否在指定的範圍限制內對元素應用樣式。
- **`:required` 僞類**：該僞類的目標是具有 `required` 屬性的 `input` 元素。 它提示用戶，他們必須填寫該字段才能提交表單。
- **`:optional` 僞類**：該僞類爲非必填且可留空的輸入元素應用樣式。
...
- **`:target` 僞類**：該僞類用於對作爲 URL 片段目標的元素應用樣式。
...
- **`:last-child` 僞類**：該僞類選擇父元素或文檔中的最後一個元素。
- **`:only-child` 僞類**：該僞類選擇父元素或文檔中的唯一元素。
...
- **`rgb()` 函數**：RGB 代表紅、綠、藍——光的三原色。 這三種顏色以不同的強度組合在一起，創造出多種顏色。 `rgb()` 函數允許你使用 RGB 顏色模型定義顏色。
- **`rgba()` 函數**：該函數添加了第四個值——alpha——用於控制顏色的透明度。
- **`hsl()` 函數**：HSL 代表色相、飽和度和亮度——是定義顏色的三個關鍵要素。
- **`hsla()` 函數**：該函數添加了第四個值——alpha——用於控制顏色的不透明度。
